...
def validate_tables_and_fields(table, fields):
    ...
    DROPPED with a warning rather than aborting the whole run. Returns the
    cleaned, comma-separated field string to use for the query.
    """
    ...
    if not valid:
        print(f"{Fore.RED}{Style.BRIGHT}ERROR: No valid fields left for Table '{table}' — {Style.RESET_ALL}exiting.")
        exit(1)
