...
# Builds the Gemini client which is used to send requests to the Gemini API
# and have conversations with the model
gemini_client = genai.Client(api_key=_keys.GEMINI_API_KEY)
...
unformatted_query_context = EXECUTOR.get_log_query_from_agent(gemini_client, user_message, model=model)
...
number_of_tokens = MODEL_MANAGEMENT.count_tokens(gemini_client, threat_hunt_messages, model)
